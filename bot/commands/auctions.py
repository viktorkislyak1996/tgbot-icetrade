...
from bot.messages import receive_product_message


async def keywords(message: types.Message, session: sessionmaker) -> None:
    user = await receive_user_from_message(message, session)

    # tracking_products = await get_all_tracking_products_by_user_id(user.id, session)
    ...
    for auction in tracking_auctions:
        ...
        response_message = receive_product_message(auction)
