...
from bot.messages import receive_db_auction_message
...
async def keywords(message: types.Message, session: sessionmaker) -> None:
    ...
    for auction in tracking_auctions:
        ...
        response_message = receive_db_auction_message(auction)
    ...
    if not tracking_auctions:
        await message.answer(
            ...
            'в полученном сообщении, нажмите на кнопку *"Запустить отслеживание"*',
            parse_mode=ParseMode.MARKDOWN
        )
